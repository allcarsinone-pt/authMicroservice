  ...
  async execute (request, response) {
    ...
    try {
      ...
      if (!resultAUth.success) {
        LogService.execute({ from: 'authEditService', data: resultAUth.error.message, date: new Date(), status: 'error' }, this.logService)
        return response.status(500).json({ error: resultAUth.error.message })
      }


      const { resultAUth.data.role_id, username, name, email, role_id } = request.body
