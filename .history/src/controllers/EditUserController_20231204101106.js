...
class EditUserController {
  ...
  async execute (request, response) {
    ...
    try {
      ...
      const { username, name, email, role_id } = request.body
      if (!resultEdit.data.role_id || !email || !username || !name || !role_id) {
      ...
      const user = await usecase.execute({
        resultEdit.data.role_id, username, name, address, city, postalcode, mobilephone, email, role_id
      })
