...
class PostgreUserRepository {
  ...
  async isLastAdmin (roleAdmin) {
    ...
    const result = await client.query('SELECT COUNT(users.id) as cnt FROM users INNER JOIN roles ON roles.id = users.role_id WHERE roles.id = $1', [parseint(roleAdmin)])
