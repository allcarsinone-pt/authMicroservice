...
class PostgreUserRepository {
  ...
  async create (user) {
    ...
    const query = 'INSERT INTO users ( username, name, password, address, city, postalcode AS postalCode, ' +
    'mobilephone AS mobilePhone, email, role_id AS roleId) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *'
  ...
  async edit (user) {
    const { id, username, name, email, address, city, postalCode, mobilePhone, roleId } = user
    const client = new pg.Client(this.baseURI)
    await client.connect()
