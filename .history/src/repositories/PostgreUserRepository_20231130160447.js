...
class PostgreUserRepository {
  ...
  async create (user) {
    ...
    const values = [username, name, password, address, city, postalCode, mobilePhone, email, role]
  ...
  async edit (user) {
    const { id, username, name, email, address, city, postalCode, mobilePhone, role } = user
    const client = new pg.Client(this.baseURI)
    await client.connect()
    ...
    const values = [username, name, address, city, postalCode, mobilePhone, email, role, id]
    const result = await client.query(query, values)
    await client.end()
    return new User({ ...result.rows[0] })
  }
