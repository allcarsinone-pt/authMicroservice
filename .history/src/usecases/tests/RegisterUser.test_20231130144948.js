...
describe('RegisterUserUseCase', () => {
  it('should register a new user', async () => {
    ...
    expect(result.data).toHaveProperty('roleId', 1)
  ...
  it('should return a result.failed if user already exists', async () => {
    const { sut, userRepository } = makeSut()
    userRepository.create(new User({ name: 'John Doe', username: 'johndoe123', email: 'test@test.com', password: '123456', roleId: 1 }))
    const result = await sut.execute({
      name: 'John Doe',
      username: 'johndoe123',
      email: 'test@test.com',
