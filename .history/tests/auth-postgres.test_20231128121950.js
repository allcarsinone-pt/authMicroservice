...
describe('Tests', () => {
  ...
  describe('POST /users/register', () => {
    it('should return 201 if user is registered', async () => {
      const requestBody = { username: 'test_username', email: 'test@test.com', name: 'test', password: '12345678', confirmPassword: '12345678', role: 'user' }
    ...
    it('should return 400 if password has less than 8 characters', async () => {
      const requestBody = { email: 'test@test.com', name: 'test', username: 'test_username', password: '123456', confirmPassword: '123456', role: 'user' }
      const response = await request.post('/users/register').send(requestBody)
      expect(response.status).toBe(400)
    ...
    it('should return 400 if password and confirmPassword are different', async () => {
      const requestBody = { email: 'test@test.com', name: 'test', username: 'test_username', password: '12345678', confirmPassword: '1234567', role: 'user' }
      const response = await request.post('/users/register').send(requestBody)
      expect(response.status).toBe(400)
  ...
  describe('POST /users/login', () => {
    ...
    it('should return 200 if user is logged in', async () => {
      ...
      await userRepository.create(new User('John Doe', 'test@test.com', hash, 'admin'))
    ...
    it('should return 400 if email is wrong', async () => {
      const salt = bcrypt.genSaltSync(10)
      const hash = bcrypt.hashSync('12345678', salt)
      await userRepository.create(new User('John Doe', 'test1@test.com', hash, 'admin'))
      const requestBody = { email: 'test@test.com', password: '12345678' }
      const response = await request.post('/users/login').send(requestBody)
      expect(response.status).toBe(400)
