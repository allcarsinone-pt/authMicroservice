...
describe('Tests', () => {
  ...
  describe('POST /users/register', () => {
    it('should return 201 if user is registered', async () => {
      const requestBody = { username: 'test_username', email: 'test@test.com', name: 'test', password: '12345678', confirmPassword: '12345678', roleId: 1 }
      ...
      expect(response.body).toHaveProperty('role_id', requestBody.roleId)
      expect(response.body).not.toHaveProperty('password')
    })
    it('should return 400 if some parameter is missing', async () => {
      const requestBody = { name: 'test', username: 'test_username', password: '12345678', confirmPassword: '12345678', roleId: 1 }
      const response = await request.post('/users/register').send(requestBody)
    ...
    it('should return 400 if password has less than 8 characters', async () => {
      const requestBody = { email: 'test@test.com', name: 'test', username: 'test_username', password: '123456', confirmPassword: '123456', roleId: 1 }
      const response = await request.post('/users/register').send(requestBody)
      expect(response.status).toBe(400)
    ...
    it('should return 400 if password and confirmPassword are different', async () => {
      const requestBody = { email: 'test@test.com', name: 'test', username: 'test_username', password: '12345678', confirmPassword: '1234567', roleId: 1 }
      const response = await request.post('/users/register').send(requestBody)
      expect(response.status).toBe(400)
      expect(response.body).toHaveProperty('message', 'Passwords don t match')
    })
    it('should return 400 if email is invalid', async () => {
      const requestBody = { email: 'test', name: 'test', username: 'test_username', password: '12345678', confirmPassword: '12345678', roleId: 1 }
      const response = await request.post('/users/register').send(requestBody)
      expect(response.status).toBe(400)
  ...
  describe('POST /users/login', () => {
    ...
    it('should return 200 if user is logged in', async () => {
      ...
      const user = { email: 'test@test.com', name: 'John Doe', username: 'test_username', password: hash, roleId: '2' }
    ...
    it('should return 400 if email is wrong', async () => {
      const salt = bcrypt.genSaltSync(10)
      const hash = bcrypt.hashSync('12345678', salt)
      const user = { email: 'test@test.com', name: 'John Doe', username: 'test_username', password: hash, roleId: '1' }
      await userRepository.create(new User(user))
  ...
  describe('GET /users/validate', () => {
    ...
    beforeAll(async () => {
      ...
      const user = { email: 'test1@test.com', name: 'John Doe', username: 'test_username', password: hash, roleId: 2 }
      await userRepository.create(new User(user))
  ...
  describe('GET /users/delete', () => {
    ...
    beforeAll(async () => {
      const salt = bcrypt.genSaltSync(10)
      const hash = bcrypt.hashSync('12345678', salt)
      const user = { email: 'test2@test.com', name: 'John Doe', username: 'test_username2', password: hash, roleId: 2, id: 1 }
      await userRepository.create(new User(user))
