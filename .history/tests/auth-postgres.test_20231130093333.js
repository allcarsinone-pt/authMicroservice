...
describe('Tests', () => {
  ...
  describe('POST /users/login', () => {
    ...
    it('should return 400 if email is wrong', async () => {
      ...
      const user = { email: 'test1@test.com', name: 'teJohn Doest', username: 'test_username', password: hash, role_id: '1' }
    ...
    it('should return 400 if password is wrong', async () => {
      ...
      const user = { email: 'test1@test.com', name: 'teJohn Doest', username: 'test_username', password: '1111', role_id: '1' }
      await userRepository.create(new User(user))
      const requestBody = { email: 'test@test.com', password: '12345678' }
      const response = await request.post('/users/login').send(requestBody)
      expect(response.status).toBe(400)
