...
class LoginController {
  ...
  async execute (req, res) {
    ...
    if (!result.success) {
      await LogService.execute({from: 'authService', data: result.error.message, date: new Date(),status: 'error'})
    ...
    await LogService.execute({from: 'authService', data: `${result.data.id}-${result.data.role} logs in`, date: new Date(),status: 'info'})
